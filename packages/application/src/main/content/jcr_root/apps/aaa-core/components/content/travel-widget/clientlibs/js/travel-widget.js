...
function checkCruise(el) {
  var destination = $('select[name=destination]', el),
    cruiseline = $('select[name=cruiseline]', el),
...
function generateCruiseDates() {
  ...
  while (currentDate < maxDate) {
    result += '<option value="' + (currentDate.getMonth() + 1) + 'x' + currentDate.getFullYear() + '">' + monthNames[currentDate.getMonth()] + ' ' + currentDate.getFullYear() + '</option>';
...
$j(function ($) {
  ...
  $(".rs_mvp_form").submit(function (e) {
    ...
    window.location.href = "https://southernnewengland.membervacationportal.com/?keyword=" + mvp_kw + "&start=" + mvp_start + "&end=" + mvp_end + "";
    return false;
  });
...
function updateTravelers(menu) {
  ...
  var dropdown = "#" + name.toString() + "-drop",
    cabinClass = "#" + name.toString() + "-class";
  $(dropdown).text(function () {
    var adults = $(this).siblings('.dropdown-content').find('input[name=rs_adults]').val(),
      children = $(this).siblings('.dropdown-content').find('input[name=rs_children]').val(),
      travelers = +adults + +children,
      cabin = $(cabinClass).find(":selected").text();
  ...
  if ($('.travel-widget').length > 0) {
    ...
    $('.plus_button').unbind().click(function () {
      var menu = $(this).parents().eq(4).attr('id'),
        number = $(this).parent().siblings('input'),
        value = number.val();
      if (value < 8) {
        value++;
        number.val(value);
