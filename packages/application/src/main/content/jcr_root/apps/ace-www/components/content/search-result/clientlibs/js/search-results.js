$j(function($) {
	...
	function getResultData(searchkeyValue, navigation, currentPage) {
		...
		var url = "https://www.googleapis.com/customsearch/v1?key=" + apikey
				+ "&cx=" + engineid + "&q=" + searchkeyValue + "&start="
				+ startIndex + "&siteSearch=" + domain + "&callback=?";
	...
	function SearchCompleted(response) {
		if (response.error) {
			...
			for (var i = 0; i < response.items.length; i++) {
				...
				html += "<div class='result-item'><a href='"
						+ item.link
						+ "' class='link-btn link-lg roboto-medium font-f'>"
						+ responseTitle
						+ "</a><a href='"
						+ item.link
						+ "' class='link-btn link-lg roboto-light font-h result-link'>"
						+ responseUrl + "</a><p class='roboto-light font-h'>"
						+ responseData + "</p></div>";
			}
			...
			for (var i = defaultPage; i < defaultTotalPages; i++) {
				...
				navigation_html += '<a href="javascript:void(0);" class="pagination-pageclick">'
						+ (i) + '</a>';
				navigation_html += '</li>';
			}
