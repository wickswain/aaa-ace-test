...
public class SendTemplatedEmailProcess implements WorkflowProcess {
    ...
    private List<UserDetailsBean> getRecipientsList(String groupName, String userMapperService) {
        ...
        UserManager userManager = getUserManager(userMapperService);

        try {
            if (StringUtils.isNotBlank(groupName) && userManager != null) {
                ...
                while (iterator.hasNext()) {
                    ...
                    if (userAuthorization.hasProperty("./profile/email")) {
                        userDetails.setEmail(
                                userAuthorization.getProperty("./profile/email").toString());
                    }

                    if (userAuthorization.hasProperty("./profile/givenName")) {
                        userDetails.setEmail(
                                userAuthorization.getProperty("./profile/givenName").toString());
                    }

                    recipientList.add(userDetails);
                }
            } else {
                log.error("Group name or user manager is empty.");
            }
    ...
    private UserManager getUserManager(String userMapperService) {
        log.info("Fetch the user manager.");
        ...
        try {
            ...
            log.error("Error occured while fetching the admin session from user mapper service: "
                    + e.toString());
            e.printStackTrace();
        } finally {
            if (adminSession != null) {
