...
public class NavigationProvider extends WCMUsePojo {
    ...
    private static final int FIRST_LEVEL_NAVIGATION_CHILD_MAX_COUNT = 8;
    ...
    /*
     * This method iterates over all the child nodes of the parent page. For
     * current node iterate each child set currengtPage,hasChildren and list of
     * children of that child in NavigationVO object based on node properties.
     */
    ...
    private boolean isTilePage(Page page) {
        if (page.getProperties().containsKey(PAGE_TYPE_KEY)
                && page.getProperties().get(PAGE_TYPE_KEY).equals(PAGE_TYPE_VALUE)) {
            return true;
        }
        return false;
    }
