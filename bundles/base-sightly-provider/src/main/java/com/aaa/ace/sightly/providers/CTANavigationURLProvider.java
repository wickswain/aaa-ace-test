...
public class CTANavigationURLProvider extends WCMUsePojo {
	...
	public void activate() throws Exception {
		...
		String regionName = StringUtils.isNotBlank(regionNameFromURL)?regionNameFromURL:get("regionName", String.class);
		log.info("queryStringKeys from component: " + keys);
		if(StringUtils.isNotBlank(url) && StringUtils.isNotEmpty(url)){
			for(int i = 0; i < keyArray.length; i++){
				if(url.contains(CTANavigationURLProvider.DOMAIN_PLACE_HOLDER) && url.contains(CTANavigationURLProvider.CLUB_PLACE_HOLDER)){
		...
		if(StringUtils.isNotBlank(key)){
			if(getRequest().getParameter(key)!=null){
				...
				url = (url.contains(CTANavigationURLProvider.EXCLAMATION_CHARACTER)?
							url.concat(CTANavigationURLProvider.AMPERSAND_CHARACTER):url.concat(CTANavigationURLProvider.EXCLAMATION_CHARACTER))
							   .concat(key).concat(CTANavigationURLProvider.EQUALS_CHARACTER).concat(queryStringValue);
			}
			else
